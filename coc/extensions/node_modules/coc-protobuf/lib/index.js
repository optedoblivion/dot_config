...
async function activate(context) {
    ...
    context.subscriptions.push(coc_nvim_1.languages.registerDefinitionProvider(mode, new DefinitionProvider_1.ProtobufDefinitionProvider()));
}
...
function getMessageDefinitionStartPosition(messageName, fileContent) {
    const pattern = new RegExp(`message ${messageName}`);
    const allLines = fileContent.split(os_1.EOL);
    let match = null;
    let matchLineOffset = 0;
    ...
    for (let i = 0; i < allLines.length; i++) {
        ...
        if (match) {
            matchLineIndex = i;
            matchLineOffset = match.index;
            break;
        }
    }
    if (match) {
        return vscode_languageserver_protocol_1.Position.create(matchLineIndex, matchLineOffset);
    }
    return null;
}
class ProtobufDefinitionProvider {
    async provideDefinition(document, position) {
        ...
        if (messageStartPostion) {
            ...
            return vscode_languageserver_protocol_1.Location.create(document.uri, targetRange);
        }
        return null;
    }
}
exports.ProtobufDefinitionProvider = ProtobufDefinitionProvider;
